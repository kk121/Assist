...
import com.krishna.assist.data.DBOpenHelper;
import com.krishna.assist.data.pojo.DBPojo;
import com.krishna.assist.data.pojo.Table;
...
public class DBUtils {
    ...
    public static String executeQuery(Context context, DBPojo dbPojo, String query) {
        StringBuilder sb = new StringBuilder();
        if (dbPojo.getDbVersion() > 0) {
            DBOpenHelper dbOpenHelper = new DBOpenHelper(context, dbPojo.getDbName(), null, dbPojo.getDbVersion());
            SQLiteDatabase db = dbOpenHelper.getReadableDatabase();
            Cursor cursor = db.rawQuery(query, null);
            sb.append("[");
            if (cursor != null && cursor.moveToFirst()) {
                addTableHeaders(cursor, sb);
                addTableRows(cursor, sb);
                cursor.close();
            }
        }
        return sb.toString();
    ...
    private static void addTableRows(Cursor cursor, StringBuilder sb) {
        do {
            sb.append(", {");
    ...
    private static void addTableHeaders(Cursor cursor, StringBuilder sb) {
        sb.append("{");
        for (int i = 0; i < cursor.getColumnCount(); i++) {
